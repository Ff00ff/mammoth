...
export class ColumnWrapper<Name, BaseType, SelectType, InsertType, UpdateType> {
  ...
  private aggregate<Type = BaseType>(aggregateType: AggregateType) {
    return new AggregateColumnWrapper<Name, Type, NotNull<Type>, NotNull<Type>, NotNull<Type>>(
      aggregateType,
      this.table,
      this.column,
      this.camelCaseName,
...
export class Column<T, IT = Null<T>, ST = Null<T>, UT = Null<T>> {
  dataType?: string;
  ...
  null(): Column<toNull<T>, toNull<T>, toNull<T>> {
